...
(
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            rollback_rebuild_main <= 1'b0;
        end
        else if (((lp_state_sub_domain == LP_MAIN_DEACT_STG1) ||
                  (lp_state_sub_domain == LP_MAIN_DEACT_STG2)) &&
                 rx_req_main_sync.deny_or) begin
            rollback_rebuild_main <= 1'b1;
        end
        else if (rollback_rebuild_main && rx_req_main_sync.stg1_req_and) begin
            rollback_rebuild_main <= 1'b0;
        end
    end
    ...
    always_comb begin
        ...
        case (lp_state_sub_domain)
            LP_MAIN_STOP: begin
                if (rx_req_main_sync.stg1_req_or) // start activation as soon as the main domain requests power-up
            ...
            LP_MAIN_RUN: begin
                if (rx_req_main_sync.state != LP_LEVEL0_NOP) // enter deact once the broadcast state requests power-down
                    lp_state_sub_domain_n = LP_MAIN_DEACT_STG1;
            end
            LP_MAIN_DEACT_STG1: begin
                if (rx_req_main_sync.deny_or)
                    lp_state_sub_domain_n = LP_MAIN_RUN;
                else if (~(rx_req_sub_sync.stg1_req_or || tx_req_sub_cdc.stg1_req_or)) // sub domain STG1 handshake
                    lp_state_sub_domain_n = LP_MAIN_DEACT_STG2;
            end
            LP_MAIN_DEACT_STG2: begin
                if (rx_req_main_sync.deny_or)
                    lp_state_sub_domain_n = LP_MAIN_RUN;
                else if (~(rx_req_sub_sync.stg2_req_or || tx_req_sub_cdc.stg2_req_or)) //sub domain STG2 handshake
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            case (lp_state_sub_domain)
                ...
                LP_MAIN_RUN: begin
                    tx_req_main_cdc <= lwnoc_lp_req_signal_t'{
                        stg1_req_or  : (rollback_rebuild_main ||
                                        ((rx_req_main_sync.state == LP_LEVEL0_NOP) &&
                                         rx_req_sub_sync.stg1_req_and &&
                                         ~rx_req_main_sync.stg1_req_and)) ?
                                         rx_req_sub_sync.stg1_req_and :
                                         rx_req_main_sync.stg1_req_and,
                        stg1_req_and : (rollback_rebuild_main ||
                                        ((rx_req_main_sync.state == LP_LEVEL0_NOP) &&
                                         rx_req_sub_sync.stg1_req_and &&
                                         ~rx_req_main_sync.stg1_req_and)) ?
                                         rx_req_sub_sync.stg1_req_and :
                ...
                LP_MAIN_DEACT_STG1, LP_MAIN_DEACT_STG2: begin
                    if (rx_req_main_sync.deny_or) begin
                        tx_req_main_cdc <= lwnoc_lp_req_signal_t'{
                            stg1_req_or  : 1'b1,
                            stg1_req_and : 1'b1,
                            stg2_req_or  : 1'b1,
                            ...
                            deny_or      : 1'b1
                             ...
                             rx_req_sub_sync.stg1_req_and &&
                             ~rx_req_main_sync.stg1_req_and) begin
                        tx_req_main_cdc <= lwnoc_lp_req_signal_t'{
                            stg1_req_or  : 1'b1,
                            stg1_req_and : 1'b1,
                            stg2_req_or  : 1'b1,
