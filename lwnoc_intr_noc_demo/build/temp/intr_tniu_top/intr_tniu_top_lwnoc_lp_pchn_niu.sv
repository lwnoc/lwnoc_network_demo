...
(
    ...
    wire back_to_stable_run = stg1_all && (~deny_any);
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        else if ((fsm_cs == LP_PCH_DEACT_STG1) && deny_any) begin
            request_failed <= 1'b1;
        end
    end
    ...
    assign tx_stg1_req = (fsm_cs == LP_PCH_ACT_STG1) || (fsm_cs == LP_PCH_ACT_STG2) || (fsm_cs == LP_PCH_RUN) || (fsm_cs == LP_PCH_DENY);
    ...
    // The pchannel master consumes deny from the network but must not reflect it
    // back into the hub, otherwise a transient deny becomes self-sustaining.
    assign tx_stg1_deny = 1'b0;
