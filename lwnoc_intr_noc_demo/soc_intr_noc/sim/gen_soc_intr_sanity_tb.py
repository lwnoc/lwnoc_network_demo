...
def iniu_apb_name(endpoint: str, suffix: str) -> str:
    return f"{endpoint}_{endpoint}_sys_apb_porting_{endpoint}_sys_apb_porting_{suffix}"


def iniu_intr_name(endpoint: str) -> str:
    return f"{endpoint}_{endpoint}_sys_v_interrupt_porting_{endpoint}_sys_v_interrupt_porting_v_interrupt"
...
def classify_input_port(name: str) -> str:
    if name in {"clk_noc_up", "clk_noc_dn", "rst_noc_up_n", "rst_noc_dn_n"}:
        return "procedural"
    if "_sys_apb_porting_" in name and name.endswith(("_p_addr", "_p_enable", "_p_wdata", "_p_write", "_p_sel")):
        return "procedural"
    if "_sys_v_interrupt_porting_" in name and name.endswith("_v_interrupt"):
        return "procedural"
    if name.endswith("_clk_sys_porting_clk_sys_clk"):
        return "assign_clk"
    if name.endswith("_rst_sys_n_porting_rst_sys_n_rst_n"):
        return "assign_rst"
    if name.endswith("_pchannel_porting_pchannel_preq"):
        return "assign_on"
    if name.endswith("_pchannel_porting_pchannel_pstate"):
        return "assign_power_on"
    if name.endswith("_timeout_val_porting_timeout_val"):
        return "assign_timeout"
    if name.endswith("_iniu_src_id_porting_iniu_src_id"):
        return "assign_iniu_id"
    if name.endswith("_tniu_tgt_id_porting_tniu_tgt_id"):
...
def generate_tb() -> str:
    ...
    for row in inius:
        endpoint = row["name"]
        signal = f"{endpoint}_{endpoint}_sys_iniu_src_id_porting_{endpoint}_sys_iniu_src_id_porting_iniu_src_id"
        if signal in port_names:
            lines.append(f"    assign {signal} = 8'd{iniu_id_map[endpoint]};")

    for row in tnius:
        endpoint = row["name"]
        signal = f"{endpoint}_{endpoint}_sys_tniu_tgt_id_porting_{endpoint}_sys_tniu_tgt_id_porting_tniu_tgt_id"
        if signal in port_names:
    ...
    lines.extend(
        [
            ...
            "        int unsigned step;",
            "        begin",
            "            for (step = 0; step < 16; step++) begin",
            "                @(posedge clk_sys);",
            "                $display(",
            "                    \"[PROBE] %s t=%0t iniu_v=%0b iniu_r=%0b iniu_tgt=%0d ring_p_v=%0b ring_p_tgt=%0d tniu_v=%0b tniu_r=%0b tniu_tgt=%0d vint=%0b\",",
            "                    phase,",
            "                    $time,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_iniu.iniu_top.req_valid,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_iniu.iniu_top.req_ready,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_iniu.iniu_top.req_tgtid,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_iniu_TO_cpu_ss_tniu_SIG_pring_out_if_pring_out_if_pring_out_if_valid,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_iniu_TO_cpu_ss_tniu_SIG_pring_out_if_pring_out_if_pring_out_if_tgtid,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_tniu.tniu_top.req_valid,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_tniu.tniu_top.req_ready,",
            "                    dut.u_soc_intr_noc_wrap.cpu_ss_tniu.tniu_top.req_tgtid,",
            "                    cpu_ss_tniu_cpu_ss_tniu_sys_v_interrupt_porting_cpu_ss_tniu_sys_v_interrupt_porting_v_interrupt[TGT_INTR_BASE]",
            "                );",
            "            end",
            "        end",
            "    endtask",
