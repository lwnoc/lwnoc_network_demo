...
PD_WRAP_CORE = ROOT / "build_logic" / "soc_intr_noc_wrap_pd" / "filelist.f"
FLAT_TOKENS = (
    "intr_niu_flat.svh",
...
def _expected_dv_leaf_tokens() -> list[str]:
    tokens: list[str] = []
    for entry in RING_PLAN:
        ...
        if node_kind in {"iniu", "tniu", "sink"}:
...
def main() -> int:
    ...
    dv_wrapper_has_logic_top = _has_token(DV_WRAPPER, "build_logic/soc_intr_ring_top/filelist.f")
    ...
    pd_core_has_wrap_core = _has_token(PD_CORE, "build_logic/soc_intr_noc_wrap_pd/filelist.f")
    dv_logic_top_has_sys_payload = any("build_logic/" in line and "_sys/" in line for line in _safe_lines(DV_LOGIC_TOP))
    dv_logic_top_has_generated_leaf = _has_token(DV_LOGIC_TOP, "build_logic/soc_intr_ring_top/aon_ss_iniu.v")
    dv_logic_top_has_top_module = _has_token(DV_LOGIC_TOP, "build_logic/soc_intr_ring_top/soc_intr_ring_top.v")
    dv_missing_leaf_tokens = _missing_tokens(DV_LOGIC_TOP, _expected_dv_leaf_tokens())
    dv_missing_sys_dirs = _missing_tokens(
        DV_LOGIC_TOP,
        [f"build_logic/{path.name}/" for path in sorted((ROOT / "build_logic").glob("*_sys"))],
    )
    dv_logic_top_has_full_leaf_closure = not dv_missing_leaf_tokens
    dv_logic_top_has_full_sys_closure = not dv_missing_sys_dirs
    ...
    print(f"dv_wrapper_refs_logic_top: {'yes' if dv_wrapper_has_logic_top else 'no'}")
    ...
    print(f"dv_logic_top_has_full_sys_closure: {'yes' if dv_logic_top_has_full_sys_closure else 'no'}")
