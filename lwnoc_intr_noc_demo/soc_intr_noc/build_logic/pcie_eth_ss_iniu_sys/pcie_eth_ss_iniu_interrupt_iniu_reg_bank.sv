module pcie_eth_ss_iniu_interrupt_iniu_reg_bank
...
#(
    ...
    // Parity error: set when a read-back parity mismatch is detected
    ...
    // pulse mode CSR: 1=pulse, 0=level (default) with parity protection
    logic [INTERRUPT_NUM/32-1:0] intr_pulse_parity;
    generate
        for(genvar i = 0; i < INTERRUPT_NUM/32; i=i+1) begin : gen_intr_pulse_mode
            always_ff @(posedge clk or negedge rst_n) begin
                if (~rst_n) begin
                    intr_pulse_mode[i*32 +: 32] <= 32'h0;
                    intr_pulse_parity[i]         <= 1'b0;
                end
                else if (p_write && p_sel && p_enable &&
                         (p_addr == (INTR_PULSE_MODE_BASE_ADDR + 32'(i*4)))) begin
                    intr_pulse_mode[i*32 +: 32] <= p_wdata;
                    intr_pulse_parity[i]         <= ^p_wdata;
                end
            end
        end
    endgenerate

    // Parity checker: on any pulse-mode read, compare recomputed parity
    logic pulse_read_active;
    logic [31:0] pulse_read_data;
    logic pulse_parity_stored;
    assign pulse_read_active = ~paddr_lut_en && p_sel && p_enable && ~p_write;

    always_comb begin
        pulse_read_data = 32'hfffffffe;
        pulse_parity_stored = 1'b0;
        for (int unsigned i = 0; i < INTERRUPT_NUM/32; i++) begin
            if (p_addr == (INTR_PULSE_MODE_BASE_ADDR + 32'(i*4))) begin
                pulse_read_data = intr_pulse_mode[i*32 +: 32];
                pulse_parity_stored = intr_pulse_parity[i];
            end
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        else if (pulse_read_active && (^pulse_read_data != pulse_parity_stored)) begin
            parity_err <= 1'b1;
        end
    end
