module pcie_eth_ss_iniu_interrupt_iniu_event_recorder
...
#(
    ...
    assign event_xor      = interrupt ^ event_state;
    assign event_gate     = (enable && ~partial_rst && (~stall || event_state)) & event_xor;
    // pulse mode: suppress falling-edge events (interrupt=0 while intr_type=1)
    assign event_gate_eff = event_gate & (~intr_type | interrupt);
    ...
    always_ff @(posedge clk or negedge rst_n ) begin
        ...
        else if(intr_type && event_normal && ~event_gate_eff)begin
            // pulse mode: reset event_state after drain so next rising edge fires
            event_state <= 1'b0;
        end
        else if(event_gate_eff)begin
