module aon_ss_tniu_interrupt_tniu_async_sys_side

    import lwnoc_lp_define_package::*;
    import lwnoc_lp_struct_package::*;
    import aon_ss_tniu_interrupt_tniu_pkg::*;
#(
    parameter integer unsigned  INTERRUPT_NUM       = `aon_ss_tniu_INTR_TNIU_INTERRUPT_MAX_NUM,
    localparam integer unsigned INTERRUPT_NUM_WIDTH = $clog2(INTERRUPT_NUM),
    parameter integer unsigned  TIME_OUT_WIDTH      = `aon_ss_tniu_INTR_TNIU_TIME_OUT_WIDTH,
    parameter integer unsigned  ASYNC_FIFO_DEPTH    = `aon_ss_tniu_INTR_TNIU_SYS_ASYNC_FIFO_DEPTH,
    parameter logic             FUSA_ECC_EN         = `aon_ss_tniu_INTR_TNIU_FUSA_ECC_EN,
    parameter integer unsigned  ERR_INT_CNT_WIDTH   = `aon_ss_tniu_INTR_TNIU_ERR_INT_CNT_WIDTH,
...
//===========================================================================
    ...
    generate
        if(INTERRUPT_NUM==4096)begin
            aon_ss_tniu_spram_4096x26 u_sp_4096_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
        else if(INTERRUPT_NUM==2048)begin
            aon_ss_tniu_spram_2048x26 u_sp_2048_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
        else if(INTERRUPT_NUM==1024)begin
            aon_ss_tniu_spram_1024x26 u_sp_1024_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
        else if(INTERRUPT_NUM==512)begin
            aon_ss_tniu_spram_512x26 u_sp_512_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
        else if(INTERRUPT_NUM==256)begin
            aon_ss_tniu_spram_256x26 u_sp_256_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
        else if(INTERRUPT_NUM==128)begin
            aon_ss_tniu_spram_128x26 u_sp_128_26 ( .clk(clk), .en(mem_en), .addr(mem_addr), .rd_data(spram_rd_ecc), .wr_data(spram_wr_ecc), .wr_bit_en(1'b1), .wr_en(mem_wren) );
        end
    endgenerate
