// Auto-generated by tools/gen_noc_niu_stubs.py.
// Review-only boundary stub with fully expanded vector widths.
// All outputs are tied to '0 so reviewers can focus on interface shape and bit width.
// Future extension: FUSA error related ports will be added on this boundary in a later revision.
// Requested source top module: atb_tniu_sys
// Resolved source top module: atb_tniu_sys
// Representative config: _atb_tniu_sys_cfg (TemplateIPConfig name=atb_tniu_sys)
module atb_tniu_sys_side_stub (
    input  logic [0:0]   clk_atb_m       ,
    input  logic [0:0]   rstn_atb_m      ,
    output logic [0:0]   m_atvalid       ,
    input  logic [0:0]   m_atready       ,
    output logic [3:0]   m_atbytes       ,
    output logic [127:0] m_atdata        ,
    output logic [6:0]   m_atid          ,
    input  logic [0:0]   m_afvalid       ,
    output logic [0:0]   m_afready       ,
    input  logic [0:0]   m_syncreq       ,
    output logic [0:0]   m_atwakeup      ,
    input  logic [0:0]   preq            ,
    input  logic [1:0]   pstate          ,
    output logic [1:0]   pactive         ,
    output logic [0:0]   paccept         ,
    output logic [0:0]   pdeny           ,
    output logic [0:0]   syncreq_level   ,
    output logic [0:0]   flush_req_level ,
    input  logic [12:0]  lw_rx_req       ,
    output logic [12:0]  lw_tx_req       ,
    input  logic [12:0]  afifo_slv_rx_req,
    output logic [12:0]  afifo_slv_tx_req,
    input  logic [15:0]  wptr_async      ,
    output logic [15:0]  rptr_async      ,
    output logic [15:0]  rptr_sync       ,
    input  logic [142:0] pld_sync        ,
    input  logic [9:0]   timeout_val     
);

    // Future boundary note:
    // FUSA error reporting ports are intentionally omitted in this review stub today,
    // but this boundary is expected to grow dedicated FUSA error signaling later.

    // Review-only stub behavior: tie every output low.
    assign m_atvalid = '0;
    assign m_atbytes = '0;
    assign m_atdata = '0;
    assign m_atid = '0;
    assign m_afready = '0;
    assign m_atwakeup = '0;
    assign pactive = '0;
    assign paccept = '0;
    assign pdeny = '0;
    assign syncreq_level = '0;
    assign flush_req_level = '0;
    assign lw_tx_req = '0;
    assign afifo_slv_tx_req = '0;
    assign rptr_async = '0;
    assign rptr_sync = '0;

endmodule
