...
#(
    ...
    assign rollback_event   = tx_stg1_deny || rx_stg1_deny;
    ...
    always_comb begin
        ...
        case (stg1_fsm_state)
            LP_SLV_FSM_IDLE: begin
                if (rollback_event) stg1_fsm_state_n = LP_SLV_FSM_REVERT;
                else if (rx_stg1_req_rise) stg1_fsm_state_n = LP_SLV_FSM_REQ_ON;
            ...
            LP_SLV_FSM_REQ_OFF: begin
                if (rollback_event) stg1_fsm_state_n = LP_SLV_FSM_REVERT;
                else if (mst_stg1_ack) stg1_fsm_state_n = LP_SLV_FSM_IDLE;
            end
            LP_SLV_FSM_REVERT: begin
                if (rollback_restore_done)
                    stg1_fsm_state_n = LP_SLV_FSM_IDLE;
            end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n)
            rollback_restore_done <= 1'b0;
        else if (stg1_fsm_state != LP_SLV_FSM_REVERT)
            rollback_restore_done <= 1'b0;
        else if (mst_stg1_ack)
            rollback_restore_done <= 1'b1;
    end
