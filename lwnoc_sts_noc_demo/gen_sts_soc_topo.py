...
# Support both historical locations:
# - <demo>/soc_sts_noc/gen_sts_soc_topo.py (legacy)
# - <demo>/gen_sts_soc_topo.py (current entrypoint layout)
if (THIS_DIR / "StsNode.py").exists():
    DEMO_ROOT = THIS_DIR
else:
...
def _write_soc_compile_ingress(flow: str, build_dir: Path) -> None:
    ...
    for alias_dir in sorted(alias_map.keys()):
        ...
        ingress_lines.append(f"-f $STS_NOC_DEMO_DIR/build_logic/{alias_dir}/{filelist.name}")
