    ...
    generate
        for(genvar i = 0; i < DATA_WIDTH; i=i+1) begin : gen_sync_cell
            if (RST_VALUE[i]==0) begin
                `ifdef sts_demo_iniu_FCIP_SYNC_CELL_ARST_MODULE_NAME
                    sts_demo_iniu_`FCIP_SYNC_CELL_ARST_MODULE_NAME #(
                        .VT_TYPE(VT_TYPE            ),
                        .SYN_NUM(SYN_STAGE          )
                    ) u_sync_arst (
                        .D      (d[i]               ),
                ...
                `ifdef sts_demo_iniu_FCIP_SYNC_CELL_ASET_MODULE_NAME
                    sts_demo_iniu_`FCIP_SYNC_CELL_ASET_MODULE_NAME #(
                        .VT_TYPE(VT_TYPE            ),
                        .SYN_NUM(SYN_STAGE          )
                    ) u_sync_aset (
                        .D      (d[i]               ),
