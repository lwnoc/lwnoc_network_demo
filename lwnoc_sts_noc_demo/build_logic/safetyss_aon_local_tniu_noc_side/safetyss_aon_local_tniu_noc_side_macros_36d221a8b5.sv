`define STS_AXI_ADDR_WIDTH 32
`define STS_AXI_ARID_WIDTH 8
`define STS_AXI_ARLEN_WIDTH 8
`define STS_AXI_AWID_WIDTH 8
`define STS_AXI_AWLEN_WIDTH 8
`define STS_AXI_BID_WIDTH 8
`define STS_AXI_DATA_WIDTH 32
`define STS_AXI_RID_WIDTH 8
`define STS_AXI_SIZE_WIDTH 3
`define STS_AXI_STRB_WIDTH 4
`define STS_AXI_USER_WIDTH 8
`define STS_CTI_CHANNEL_WIDTH 40
`define STS_CTI_EVENT_WIDTH 8
`define STS_CTM_TRIG_WIDTH 32
`define STS_INIU_NUM 16
`define STS_INIU_OT_TOTAL 264
`define STS_INIU_SINGLE_OT 0
`define STS_SRC_ID_WIDTH 8
`define STS_TGT_ID_WIDTH 9
`define STS_TNIU_APB_ADDR_WIDTH 32
`define STS_TNIU_ASYNC_FIFO_DEPTH 4
`define STS_TNIU_DBG_DATA_WIDTH 32
`define STS_TNIU_DBG_TIMESTAMP_WIDTH 64
`define STS_TNIU_ERR_INT_CNT_WIDTH 16
`define STS_TNIU_FIFO_DEPTH 16
`define STS_TNIU_HAS_INIU_CTI_APB 1
`define STS_TNIU_LOCAL_APB_TGT_TYPE 2'b01
`define STS_TNIU_LOCAL_CTI_TGT_ID 9'h0E2
`define STS_TNIU_LOCAL_CTI_TGT_MASK 9'h1FF
`define STS_TNIU_LOCAL_INIU_CTI_TGT_ID 9'h122
`define STS_TNIU_LOCAL_INIU_CTI_TGT_MASK 9'h1FF
`define STS_TNIU_LOCAL_REGBANK_TGT_ID 9'h0A2
`define STS_TNIU_LOCAL_REGBANK_TGT_MASK 9'h1FF
`define STS_TNIU_REQ_FIFO_DEPTH 16
`define STS_TNIU_RSP_FIFO_DEPTH 16
`define STS_TNIU_SYNC_STAGE 2
`define STS_TNIU_TGT_TYPE_WIDTH 2
`define STS_TXN_ID_WIDTH 8
