module sts_noc_dec_node_1to5_wrap
import lwnoc_sts_pack::*;
#(
    parameter logic [(1 << TGT_ID_WIDTH)-1:0] ROUTE_VLD_TABLE = `STS_NOC_DEC_NODE_1TO5_WRAP_ROUTE_VLD_TABLE,
    parameter logic [((1 << TGT_ID_WIDTH) * $clog2(5))-1:0] ROUTE_DST_TABLE = `STS_NOC_DEC_NODE_1TO5_WRAP_ROUTE_DST_TABLE,
    parameter logic [5*TGT_ID_WIDTH-1:0] ROUTE_BASE = `STS_NOC_DEC_NODE_1TO5_WRAP_ROUTE_BASE,
    parameter logic [5*TGT_ID_WIDTH-1:0] ROUTE_MASK = `STS_NOC_DEC_NODE_1TO5_WRAP_ROUTE_MASK,
    ...
    logic [STS_RSP_WIDTH-1:0]         s_rsp_pld_s [5-1:0];
    logic [5*CHANNEL_TOTAL_WIDTH-1:0]   s_cti_channel_out_s;
    logic [5*DBG_TIMESTAMP_WIDTH-1:0] s_dbg_timestamp_s;
    logic [5*RESERVE_WIDTH-1:0]      s_reserved_bits_s;
    ...
    sts_noc_dec_node #(
        ...
        .slv_req_vld        ({s4_req_vld, s3_req_vld, s2_req_vld, s1_req_vld, s0_req_vld}                    ),
        .slv_req_rdy        ({s4_req_rdy, s3_req_rdy, s2_req_rdy, s1_req_rdy, s0_req_rdy}                    ),
        .slv_req_pld        (s_req_pld_s                                                                       ),
        .slv_rsp_vld        ({s4_rsp_vld, s3_rsp_vld, s2_rsp_vld, s1_rsp_vld, s0_rsp_vld}                    ),
        .slv_rsp_rdy        ({s4_rsp_rdy, s3_rsp_rdy, s2_rsp_rdy, s1_rsp_rdy, s0_rsp_rdy}                    ),
        .slv_rsp_pld        (s_rsp_pld_s                                                                       ),
        .mst_cti_channel_in (mst_cti_channel_in                                                                ),
        .mst_cti_channel_out(mst_cti_channel_out                                                               ),
        .slv_cti_channel_in ({s4_cti_channel_in, s3_cti_channel_in, s2_cti_channel_in, s1_cti_channel_in, s0_cti_channel_in}),
        .slv_cti_channel_out(s_cti_channel_out_s                                                               ),
        .mst_dbg_timestamp  (mst_dbg_timestamp                                                                 ),
        .slv_dbg_timestamp  (s_dbg_timestamp_s                                                                 ),
        .slv_dbg_data       ({s4_dbg_data, s3_dbg_data, s2_dbg_data, s1_dbg_data, s0_dbg_data}               ),
