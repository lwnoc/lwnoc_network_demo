module sts_noc_aon_ss_iniu_sts_iniu_rd_channel
...
sts_noc_aon_ss_iniu_sts_iniu_addr_map #(
    .ENTRY_NUM      (ADDR_MAP_ENTRY_NUM),
    .ADDR_START_TABLE(ADDR_MAP_START_TABLE),
    .ADDR_END_TABLE(ADDR_MAP_END_TABLE),
...
assign ar_in_hold_vld = 1'b0;
assign ar_in_hold_pld = '0;
assign ar_in_sel_pld  = upstrm_ar_pld;
assign fifo_in_ar_vld = upstrm_ar_vld;
assign upstrm_ar_rdy  = fifo_in_ar_rdy;
...
fcip_reg_slice #(
    ...
    .clk   (clk             ),
    .rst_n (rst_n           ),
    .s_vld (fifo_in_ar_vld  ),
    .s_rdy (fifo_in_ar_rdy  ),
    .s_pld (fifo_in_ar_pld  ),
    .m_vld (fifo_out_ar_vld ),
    .m_rdy (fifo_out_ar_rdy ),
    .m_pld (fifo_out_ar_pld )
);
...
assign ar_fifo_issue_vld = fifo_out_ar_vld;
assign ar_stage_seen = ar_fifo_issue_vld;
assign ar_hold_vld   = ar_fifo_issue_vld;
assign ar_hold_pld   = fifo_out_ar_pld;

assign alloc_vld = ar_fifo_issue_vld && out_req_rdy && alloc_rdy;
assign alloc_id  = fifo_out_ar_pld.cmn.txn_id;

assign fifo_out_ar_rdy = ar_fifo_issue_vld && out_req_rdy && alloc_rdy;

assign complete_idx = in_rsp_pld.cmn.txn_id;

assign out_req_vld = ar_fifo_issue_vld && alloc_rdy;

always_comb begin : out_req_pld_map
    out_req_pld = 'b0;
    out_req_pld.cmn        = fifo_out_ar_pld.cmn;
    out_req_pld.cmn.txn_id = rd_id_alloc; //remaped txn id
    out_req_pld.req        = fifo_out_ar_pld.req;
end
