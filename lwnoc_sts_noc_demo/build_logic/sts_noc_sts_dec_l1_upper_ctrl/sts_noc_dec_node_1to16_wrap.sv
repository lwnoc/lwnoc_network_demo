module sts_noc_dec_node_1to16_wrap
import lwnoc_sts_pack::*;
#(
    parameter logic [(1 << TGT_ID_WIDTH)-1:0] ROUTE_VLD_TABLE = `STS_NOC_DEC_NODE_1TO16_WRAP_ROUTE_VLD_TABLE,
    parameter logic [((1 << TGT_ID_WIDTH) * $clog2(16))-1:0] ROUTE_DST_TABLE = `STS_NOC_DEC_NODE_1TO16_WRAP_ROUTE_DST_TABLE,
    parameter logic [16*TGT_ID_WIDTH-1:0] ROUTE_BASE = `STS_NOC_DEC_NODE_1TO16_WRAP_ROUTE_BASE,
    parameter logic [16*TGT_ID_WIDTH-1:0] ROUTE_MASK = `STS_NOC_DEC_NODE_1TO16_WRAP_ROUTE_MASK,
    ...
    // ---- S14 Reserved Bits ---------------------------------------------
    output  logic [RESERVE_WIDTH-1:0]        s14_reserved_bits ,

    // ---- S15 REQ -------------------------------------------------
    output  logic                               s15_req_vld         ,
    input   logic                               s15_req_rdy         ,
    output  logic [STS_REQ_WIDTH-1:0]           s15_req_pld         ,

    // ---- S15 RSP -------------------------------------------------
    input   logic                               s15_rsp_vld         ,
    output  logic                               s15_rsp_rdy         ,
    input   logic [STS_RSP_WIDTH-1:0]           s15_rsp_pld         ,

    // ---- S15 CTI -------------------------------------------------
    input   logic [CHANNEL_TOTAL_WIDTH-1:0]       s15_cti_channel_in  ,
    output  logic [CHANNEL_TOTAL_WIDTH-1:0]       s15_cti_channel_out ,

    // ---- S15 DBG -------------------------------------------------
    output  logic [DBG_TIMESTAMP_WIDTH-1:0]     s15_dbg_timestamp   ,
    input   logic [DBG_DATA_WIDTH-1:0]          s15_dbg_data        ,

    // ---- S15 Reserved Bits ---------------------------------------------
    output  logic [RESERVE_WIDTH-1:0]        s15_reserved_bits ,
    ...
    logic [STS_RSP_WIDTH-1:0]         s_rsp_pld_s [16-1:0];
    logic [16*CHANNEL_TOTAL_WIDTH-1:0]   s_cti_channel_out_s;
    logic [16*DBG_TIMESTAMP_WIDTH-1:0] s_dbg_timestamp_s;
    logic [16*RESERVE_WIDTH-1:0]      s_reserved_bits_s;
    ...
    assign s_rsp_pld_s[15] = s15_rsp_pld;
    ...
    sts_noc_dec_node #(
        ...
        .slv_req_vld        ({s15_req_vld, s14_req_vld, s13_req_vld, s12_req_vld, s11_req_vld, s10_req_vld, s9_req_vld, s8_req_vld, s7_req_vld, s6_req_vld, s5_req_vld, s4_req_vld, s3_req_vld, s2_req_vld, s1_req_vld, s0_req_vld}),
        .slv_req_rdy        ({s15_req_rdy, s14_req_rdy, s13_req_rdy, s12_req_rdy, s11_req_rdy, s10_req_rdy, s9_req_rdy, s8_req_rdy, s7_req_rdy, s6_req_rdy, s5_req_rdy, s4_req_rdy, s3_req_rdy, s2_req_rdy, s1_req_rdy, s0_req_rdy}),
        .slv_req_pld        (s_req_pld_s                                                                                                                                   ),
        .slv_rsp_vld        ({s15_rsp_vld, s14_rsp_vld, s13_rsp_vld, s12_rsp_vld, s11_rsp_vld, s10_rsp_vld, s9_rsp_vld, s8_rsp_vld, s7_rsp_vld, s6_rsp_vld, s5_rsp_vld, s4_rsp_vld, s3_rsp_vld, s2_rsp_vld, s1_rsp_vld, s0_rsp_vld}),
        .slv_rsp_rdy        ({s15_rsp_rdy, s14_rsp_rdy, s13_rsp_rdy, s12_rsp_rdy, s11_rsp_rdy, s10_rsp_rdy, s9_rsp_rdy, s8_rsp_rdy, s7_rsp_rdy, s6_rsp_rdy, s5_rsp_rdy, s4_rsp_rdy, s3_rsp_rdy, s2_rsp_rdy, s1_rsp_rdy, s0_rsp_rdy}),
        .slv_rsp_pld        (s_rsp_pld_s                                                                                                                                   ),
        .mst_cti_channel_in (mst_cti_channel_in                                                                                                                            ),
        .mst_cti_channel_out(mst_cti_channel_out                                                                                                                           ),
        .slv_cti_channel_in ({s15_cti_channel_in, s14_cti_channel_in, s13_cti_channel_in, s12_cti_channel_in, s11_cti_channel_in, s10_cti_channel_in, s9_cti_channel_in, s8_cti_channel_in, s7_cti_channel_in, s6_cti_channel_in, s5_cti_channel_in, s4_cti_channel_in, s3_cti_channel_in, s2_cti_channel_in, s1_cti_channel_in, s0_cti_channel_in}),
        .slv_cti_channel_out(s_cti_channel_out_s                                                                                                                           ),
        .mst_dbg_timestamp  (mst_dbg_timestamp                                                                                                                             ),
        .slv_dbg_timestamp  (s_dbg_timestamp_s                                                                                                                             ),
        .slv_dbg_data       ({s15_dbg_data, s14_dbg_data, s13_dbg_data, s12_dbg_data, s11_dbg_data, s10_dbg_data, s9_dbg_data, s8_dbg_data, s7_dbg_data, s6_dbg_data, s5_dbg_data, s4_dbg_data, s3_dbg_data, s2_dbg_data, s1_dbg_data, s0_dbg_data}),
    ...
    assign s15_req_pld        = s_req_pld_s;
    ...
    assign s15_cti_channel_out = s_cti_channel_out_s[15*CHANNEL_TOTAL_WIDTH +: CHANNEL_TOTAL_WIDTH];
    ...
    assign s14_reserved_bits = s_reserved_bits_s[14*RESERVE_WIDTH +: RESERVE_WIDTH];
    assign s15_dbg_timestamp  = s_dbg_timestamp_s[15*DBG_TIMESTAMP_WIDTH +: DBG_TIMESTAMP_WIDTH];
    assign s15_reserved_bits = s_reserved_bits_s[15*RESERVE_WIDTH +: RESERVE_WIDTH];
