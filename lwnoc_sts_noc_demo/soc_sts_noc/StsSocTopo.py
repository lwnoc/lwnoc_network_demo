...
# Single-chain AON topology: all leaves are partition-owned and there are no
# extra non-harden blue-chain leaves to classify in the ownership manifest.
BLUE_CHAIN_LEAF_OWNERSHIP = {}
SOC_STS_NOC_TOP_ID = "soc_sts_noc"


class StsSocLogicTopo(UhdlWrapperNode):
    """AON-single-source STS topology reconstructed from the user-described diagram.

        Logical route intent:
            aon_ss_iniu -> dec0
                dec0[0] -> vpu_ss_tniu
                dec0[1] -> dec1
                    dec1[0] -> camera_ss_tniu
                    dec1[1] -> async bridge -> dec2
                        dec2 logically fans out to dspss0..5

    The available primitive decoder is 4-way, so the logical 6-way dec2 fanout
    is implemented as dec2 + dec2_ext while preserving the diagram-level split.
    """
    ...
    def __init__(self):
        ...
        # Diagram-native single source: only AON SS is an initiator.
        ...
        self.dec2_ext = StsDec4Node(id="dec2_ext")
        ...
        # Diagram-visible TNIU leaves only.
        leaf_names = [
            "vpu_ss",
            "camera_ss",
            "dspss0",
        ...
        # Diagram-native route:
        #   aon_ss_iniu -> dec0 -> {vpu_ss, dec1}
        #   dec1 -> {camera_ss, async -> logical dec2}
        #   logical dec2 -> dspss0..5 (implemented as dec2 + dec2_ext)
