module ucie_ss0_tniu_interrupt_tniu_generate_reg_bank
    ...
    generate
        for(genvar i = 0; i < INTERRUPT_NUM/32; i=i+1) begin : gen_intr_mask
            always_ff @(posedge clk or negedge rst_n) begin
                ...
                else if (wreq_vld && (wreq_addr == (INTR_MERGE_MASK_BASE_ADDR + i * 4))) begin
                    intr_merge_mask[i*32 +: 32] <= wreq_data;
                end
            end
        end
        ...
        for(genvar i = 0; i < INTERRUPT_NUM/32; i=i+1) begin : gen_intr_pulse_mode
            always_ff @(posedge clk or negedge rst_n) begin
                ...
                else if (wreq_vld && (wreq_addr == (INTR_PULSE_MODE_BASE_ADDR + i * 4))) begin
                    intr_pulse_mode[i*32 +: 32] <= wreq_data;
                end
            end
        end
