...
//
// Limitations:
//   - threshold signals: INIU-side REQ threshold (iniu_req_threshold) and
//     TNIU-side RSP threshold (tniu_rsp_threshold) are tied to 1'b1
//     (backpressure not used on those paths). The complementary directions
//     (iniu_rsp_threshold, tniu_req_threshold) are wired as inputs to
//     support optional backpressure from the connected peer.
//   - qos / tgtid channels are not forwarded (tied 0/1 per convention).
// =============================================================================

module dti_sw2_dti_noc_switch #(
    parameter int unsigned NUM_INIU      = `dti_sw2_DTI_NOC_SWITCH_NUM_INIU      ,
    parameter int unsigned NUM_TNIU      = `dti_sw2_DTI_NOC_SWITCH_NUM_TNIU      ,
    parameter int unsigned TID_WIDTH     = `dti_sw2_DTI_NOC_SWITCH_TID_WIDTH     ,
    parameter int unsigned PAYLOAD_WIDTH = `dti_sw2_DTI_NOC_SWITCH_PAYLOAD_WIDTH ,

    // REQ routing ranges — for each TNIU port j (0..NUM_TNIU-1):
    //   REQ is forwarded to TNIU[j] when TNIU_DECMIN[j] <= srcid <= TNIU_DECMAX[j]
    //   Packed flat: slot j at bits [j*32+31 : j*32]
    parameter logic [NUM_TNIU*32-1:0] TNIU_DECMIN  = `dti_sw2_DTI_NOC_SWITCH_TNIU_DECMIN ,
    parameter logic [NUM_TNIU*32-1:0] TNIU_DECMAX  = `dti_sw2_DTI_NOC_SWITCH_TNIU_DECMAX ,

    // RSP return ranges — for each INIU port i (0..NUM_INIU-1):
    //   RSP is returned to INIU[i] when INIU_DECMIN[i] <= rsp_srcid <= INIU_DECMAX[i]
    //   Packed flat: slot i at bits [i*32+31 : i*32]
    parameter logic [NUM_INIU*32-1:0] INIU_DECMIN  = `dti_sw2_DTI_NOC_SWITCH_INIU_DECMIN ,
    parameter logic [NUM_INIU*32-1:0] INIU_DECMAX  = `dti_sw2_DTI_NOC_SWITCH_INIU_DECMAX
)(
    input  logic clk                                                ,
    input  logic rst_n                                             ,

    // ── INIU master-side: INIU feeds REQ in, receives RSP out ─────────────
    ...
    input  logic [NUM_INIU-1:0][TID_WIDTH-1:0]      iniu_req_srcid  ,  // = TTID
    ...
    // ── TNIU slave-side: switch drives REQ out, receives RSP in ──────────
    ...
    input  logic [NUM_TNIU-1:0][TID_WIDTH-1:0]      tniu_rsp_srcid  ,  // echoed TTID
    ...
    // Local parameters
    // ─────────────────────────────────────────────────────────────────────
    // Grouped payload: {payload, last, srcid, tgtid, qos}
    localparam int unsigned GPW        = PAYLOAD_WIDTH + 1 + TID_WIDTH + TID_WIDTH + 1;
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Threshold tie-off (unused in current integration)
    // ─────────────────────────────────────────────────────────────────────
    assign iniu_req_threshold = {NUM_INIU{1'b1}};
    assign tniu_rsp_threshold = {NUM_TNIU{1'b1}};

    // ─────────────────────────────────────────────────────────────────────
    // Grouped payload wire arrays for bitwise transpose mux
    // ─────────────────────────────────────────────────────────────────────
    logic [NUM_INIU-1:0][GPW-1:0] req_gpld_in ;  // grouped REQ from each INIU
    logic [NUM_TNIU-1:0][GPW-1:0] req_gpld_out;  // selected REQ to each TNIU
    logic [NUM_TNIU-1:0][GPW-1:0] rsp_gpld_in ;  // grouped RSP from each TNIU
    logic [NUM_INIU-1:0][GPW-1:0] rsp_gpld_out;  // selected RSP to each INIU

    genvar gi, gj, gk;

    generate
        for (gi = 0; gi < NUM_INIU; gi++) begin : g_req_pack
            assign req_gpld_in[gi] = {iniu_req_payload[gi], iniu_req_last[gi],
                                      iniu_req_srcid[gi], iniu_req_tgtid[gi],
                                      iniu_req_qos[gi]};
        end
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_rsp_pack
            assign rsp_gpld_in[gj] = {tniu_rsp_payload[gj], tniu_rsp_last[gj],
                                      tniu_rsp_srcid[gj], tniu_rsp_tgtid[gj],
                                      tniu_rsp_qos[gj]};
        end
    endgenerate

    // =========================================================================
    // REQ PATH: NUM_INIU sources → NUM_TNIU sinks
    // =========================================================================

    // Per-TNIU state
    logic [NUM_TNIU-1:0][NUM_INIU-1:0]   req_hit      ;  // which INIUs target TNIU[j]
    logic [NUM_TNIU-1:0][NUM_INIU-1:0]   req_grant_oh ;  // one-hot winner per TNIU
    logic [NUM_TNIU-1:0]                 req_in_pkt_q ;  // 1 = mid-packet locked
    logic [NUM_TNIU-1:0][INIU_IDX_W-1:0] req_locked_q ;  // locked winner INIU index
    logic [NUM_TNIU-1:0][INIU_IDX_W-1:0] req_rr_ptr_q ;  // RR priority pointer
        ...
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_req_hit
            for (gi = 0; gi < NUM_INIU; gi++) begin : g_req_hit_iniu
                assign req_hit[gj][gi] =
                    iniu_req_valid[gi]                                        &&
                    ($unsigned(iniu_req_srcid[gi]) >=
                        TNIU_DECMIN[gj*32 +: 32])                           &&
                    ($unsigned(iniu_req_srcid[gi]) <=
                        TNIU_DECMAX[gj*32 +: 32])                           ;
            end
        end
    endgenerate

    // ── REQ per-TNIU arbiter + stickiness ───────────────────────────────────
    generate
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_req_arb

            // Grant: when locked, drive locked winner; otherwise RR arbitrate
            always_comb begin : req_grant_comb
                req_grant_oh[gj] = '0;
                if (req_in_pkt_q[gj]) begin
                    // Mid-packet: hold locked winner (may be 0 if winner stalls)
                    if (req_hit[gj][req_locked_q[gj]]) begin
                        req_grant_oh[gj]                = '0;
                        req_grant_oh[gj][req_locked_q[gj]] = 1'b1;
                    end
                end else begin
                    // Free: round-robin scan from rr_ptr_q
                    for (int offset = 0; offset < NUM_INIU; offset++) begin
                        automatic int idx;
            ...
            always_ff @(posedge clk or negedge rst_n) begin : req_state_ff
                if (!rst_n) begin
                    ...
                    if (tniu_req_valid[gj] && tniu_req_ready[gj]) begin
                        if (tniu_req_last[gj]) begin
                            // End of packet: unlock, advance RR pointer
                            req_in_pkt_q[gj] <= 1'b0;
                            req_rr_ptr_q[gj] <= INIU_IDX_W'(req_locked_q[gj] + 1'b1);
                        end else if (!req_in_pkt_q[gj]) begin
                            // First beat of multi-beat packet: lock winner
                            req_in_pkt_q[gj] <= 1'b1;
                            for (int k = 0; k < NUM_INIU; k++) begin
            ...
            assign tniu_req_valid[gj]   = |req_grant_oh[gj];
                assign {tniu_req_payload[gj], tniu_req_last[gj], tniu_req_srcid[gj],
                    tniu_req_tgtid[gj], tniu_req_qos[gj]} = req_gpld_out[gj];

        end
    endgenerate

    // ── REQ bitwise transpose OR mux ────────────────────────────────────────
    generate
    ...
    // =========================================================================
    // RSP PATH: NUM_TNIU sources → NUM_INIU sinks (stateless by TID range)
    // =========================================================================
        ...
        for (gi = 0; gi < NUM_INIU; gi++) begin : g_rsp_hit
            for (gj = 0; gj < NUM_TNIU; gj++) begin : g_rsp_hit_tniu
                assign rsp_hit[gi][gj] =
                    tniu_rsp_valid[gj]                                        &&
                    ($unsigned(tniu_rsp_srcid[gj]) >=
                        INIU_DECMIN[gi*32 +: 32])                           &&
                    ($unsigned(tniu_rsp_srcid[gj]) <=
                        INIU_DECMAX[gi*32 +: 32])                           ;
            end
        end
    endgenerate

    // ── RSP per-INIU arbiter + stickiness ───────────────────────────────────
    generate
        for (gi = 0; gi < NUM_INIU; gi++) begin : g_rsp_arb
            ...
            always_comb begin : rsp_grant_comb
                ...
                if (rsp_in_pkt_q[gi]) begin
                    ...
                    for (int offset = 0; offset < NUM_TNIU; offset++) begin
                        automatic int idx;
            ...
            always_ff @(posedge clk or negedge rst_n) begin : rsp_state_ff
                if (!rst_n) begin
                    ...
                    if (iniu_rsp_valid[gi] && iniu_rsp_ready[gi]) begin
                        if (iniu_rsp_last[gi]) begin
                            ...
                            for (int k = 0; k < NUM_TNIU; k++) begin
            ...
            assign iniu_rsp_valid[gi]   = |rsp_grant_oh[gi];
                assign {iniu_rsp_payload[gi], iniu_rsp_last[gi], iniu_rsp_srcid[gi],
                    iniu_rsp_tgtid[gi], iniu_rsp_qos[gi]} = rsp_gpld_out[gi];

        end
    endgenerate

    // ── RSP bitwise transpose OR mux ────────────────────────────────────────
    generate
    ...
    generate
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_tniu_rsp_ready
            always_comb begin : tniu_rsp_ready_comb
                ...
                for (int i = 0; i < NUM_INIU; i++) begin
