// dti_noc_switch_2to1_wrap.sv — 2-to-1 per-INIU DTI switch wrapper.
// Wraps dti_noc_switch #2 with individually-named per-channel ports
// for UHDL Bundle connect compatibility (positional SmartAssign).

module dti_sw1_dti_noc_switch_2to1_wrap #(
    parameter int unsigned TID_WIDTH       = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_TID_WIDTH       ,
    parameter int unsigned PAYLOAD_WIDTH   = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_PAYLOAD_WIDTH   ,
    parameter logic [1*32-1:0] TNIU_DECMIN = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_TNIU_DECMIN     ,
    parameter logic [1*32-1:0] TNIU_DECMAX = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_TNIU_DECMAX     ,
    parameter logic [2*32-1:0] INIU_DECMIN = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_INIU_DECMIN     ,
    parameter logic [2*32-1:0] INIU_DECMAX = `dti_sw1_DTI_NOC_SWITCH_2TO1_WRAP_INIU_DECMAX
)(
    input  logic clk                                          ,
    input  logic rst_n                                        ,
    ...
    dti_sw1_dti_noc_switch #(
        ...
        .iniu_req_valid  ({iniu1_req_valid, iniu0_req_valid})              ,
        .iniu_req_payload  ({iniu1_req_payload, iniu0_req_payload})              ,
        .iniu_req_last  ({iniu1_req_last, iniu0_req_last})              ,
        .iniu_req_srcid  ({iniu1_req_srcid, iniu0_req_srcid})              ,
        .iniu_req_tgtid  ({iniu1_req_tgtid, iniu0_req_tgtid})              ,
        .iniu_req_qos  ({iniu1_req_qos, iniu0_req_qos})              ,
        .iniu_req_threshold  ({iniu1_req_threshold, iniu0_req_threshold})              ,
        .iniu_req_ready  ({iniu1_req_ready, iniu0_req_ready})              ,

        .iniu_rsp_valid  ({iniu1_rsp_valid, iniu0_rsp_valid})              ,
        .iniu_rsp_payload  ({iniu1_rsp_payload, iniu0_rsp_payload})              ,
        .iniu_rsp_last  ({iniu1_rsp_last, iniu0_rsp_last})              ,
        .iniu_rsp_srcid  ({iniu1_rsp_srcid, iniu0_rsp_srcid})              ,
        .iniu_rsp_tgtid  ({iniu1_rsp_tgtid, iniu0_rsp_tgtid})              ,
        .iniu_rsp_qos  ({iniu1_rsp_qos, iniu0_rsp_qos})              ,
        .iniu_rsp_threshold  ({iniu1_rsp_threshold, iniu0_rsp_threshold})              ,
