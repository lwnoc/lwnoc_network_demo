module pcie_eth_dti_pr_iniu_async_sys_side
    import lwnoc_lp_define_package::*;
    import lwnoc_lp_struct_package::*;
    import pcie_eth_dti_iniu_pack::*;
    #(
        parameter integer unsigned TBU_NUM              = `pcie_eth_TBU_NUM,
        parameter integer unsigned TRANSACTION_MAX_NUM  = `pcie_eth_TRANSACTION_MAX_NUM,
        parameter integer unsigned  ASYNC_FIFO_DEPTH    = `pcie_eth_INIU_ASYNC_FIFO_DEPTH,
        parameter integer unsigned  TIME_OUT_WIDTH      = `pcie_eth_INIU_TIME_OUT_WIDTH,
        ...
        parameter integer unsigned  ERR_INT_CNT_WIDTH   = `pcie_eth_INIU_ERR_INT_CNT_WIDTH,
    ...
    logic                                               async_req_almost_full_unused ;
    logic                                               rsp_async_almost_empty_unused;
    logic   [90+6+6+1+1-1:0]                            rsp_pld_vector               ;
    // FUSA ECC: AFIFO boundary encode/decode signals
    logic   [AFIFO_DATA_WIDTH-1:0]                      req_pld_fifo_in              ;
    logic   [AFIFO_DATA_WIDTH-1:0]                      rsp_pld_fifo_out             ;
    // lp
    logic                                               niu_stall                    ;
    ...
    //===========================================================================
    // async fifo req slv — ECC encode (FUSA_ECC_EN) before AFIFO write
    //===========================================================================
    assign req_pld_vector = {req_payload,req_srcid,req_tgtid,req_qos,req_last};
    // always ECC enabled
    fcip_ecc_enc #(.DATA_WIDTH(ORG_PLD_WIDTH)) u_req_afifo_ecc_enc (
        .data       (req_pld_vector ),
        .encode_data(req_pld_fifo_in)
    );
    fcip_afifo_slv #(
        ...
        .full_zero          (async_req_full_zero    ),
        .s_vld              (req_valid              ),
        .s_pld              (req_pld_fifo_in        ),
        .s_rdy              (req_ready              ),
        .almost_full        (async_req_almost_full_unused),
    ...
    //===========================================================================
    // async fifo rsp mst — ECC decode (FUSA_ECC_EN) after AFIFO read
    //===========================================================================
    fcip_afifo_mst #(
        .DATA_WIDTH         (AFIFO_DATA_WIDTH        ),
        .FIFO_DEPTH         (ASYNC_FIFO_DEPTH       )
        ...
        .idle               (rsp_async_idle         ),
        .m_vld              (rsp_valid              ),
        .m_pld              (rsp_pld_fifo_out       ),
        .m_rdy              (rsp_ready              ),
        .almost_empty       (rsp_async_almost_empty_unused),
    ...
    generate
        // always ECC enabled
        logic   rsp_afifo_sb_err_raw;
        logic   rsp_afifo_db_err_raw;
        fcip_ecc_dec #(.DATA_WIDTH(ORG_PLD_WIDTH)) u_rsp_afifo_ecc_dec (
            .encode_data(rsp_pld_fifo_out),
            .data       (rsp_pld_vector  ),
            .sb_err     (rsp_afifo_sb_err_raw),
            .db_err     (rsp_afifo_db_err_raw)
        );
        fcip_fusa_pulse_gen #(.CNT_WIDTH(ERR_INT_CNT_WIDTH)) u_pulse_rsp_sb (
            .clk(clk), .rst_n(rst_n), .err_in(rsp_afifo_sb_err_raw), .intr_out(rsp_afifo_sb_err)
        );
        fcip_fusa_pulse_gen #(.CNT_WIDTH(ERR_INT_CNT_WIDTH)) u_pulse_rsp_db (
            .clk(clk), .rst_n(rst_n), .err_in(rsp_afifo_db_err_raw), .intr_out(rsp_afifo_db_err)
        );
    endgenerate
    //===========================================================================
    // TWAKEUP generation (AXI5-Stream §2.3)
    // rsp_twakeup must stay HIGH whenever rsp_tvalid=1 (Rule 4: transmitter
    // must not lower TWAKEUP while TVALID=1 and TREADY=0).
    //
    // Two pipeline stages can hold live data independently:
    //   stage-1: fcip_afifo_mst FIFO body → tracked by ~rsp_async_idle
    //   stage-2: fcip_afifo_mst output register → tracked by rsp_valid (m_vld)
    // Once data has drained from the FIFO body into the output register,
    // rsp_async_idle goes HIGH while rsp_valid is still HIGH. OR-ing both
    // ensures rsp_twakeup_r stays HIGH until the output register is consumed.
    //
    // Force LOW during niu_partical_rst: rsp_tvalid is also forced 0 in that
    //         phase, so Rule 4 is not violated.
    //===========================================================================
    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n)                rsp_twakeup_r <= 1'b0 ;
        else if (niu_partical_rst) rsp_twakeup_r <= 1'b0 ;
        else                       rsp_twakeup_r <= ~rsp_async_idle || rsp_valid ;
    end
    assign rsp_twakeup = rsp_twakeup_r ;

    // pragma translate_off
    //===========================================================================
    // AXI5-Stream TWAKEUP SVA (IHI0051B §2.3)
    // Rule 4: transmitter must not deassert TWAKEUP while TVALID=1 TREADY=0.
    // Disabled during niu_partical_rst: rsp_tvalid is also 0 in that phase.
    //===========================================================================
    twakeup_rule4_iniu_rsp: assert property (
        @(posedge clk) disable iff (!rst_n || niu_partical_rst)
        (rsp_tvalid && !rsp_tready) |-> rsp_twakeup
    ) else $error("[SVA][twakeup_rule4_iniu_rsp] rsp_twakeup deasserted while rsp_tvalid=1 rsp_tready=0");

    twakeup_asserts_iniu_rsp_cov: cover property (
        @(posedge clk) $rose(rsp_twakeup)
    );
    // pragma translate_on
endmodule
