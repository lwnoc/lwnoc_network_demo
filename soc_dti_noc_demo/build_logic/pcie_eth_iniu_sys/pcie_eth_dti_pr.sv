...
#(
    ...
    parameter integer unsigned TTID_BASE           = 0
)(
    input   logic                                       clk                                         ,
    input   logic                                       rst_n                                       ,
    // REQ_data channel
    ...
    // TBU_NUM=1 → $clog2(1)=0 会产生 [-1:0] 空向量，用三目保底
    localparam int unsigned TBU_IDX_W = (TBU_NUM > 1) ? $clog2(TBU_NUM) : 1;
    ...
    logic [TBU_IDX_W-1  :0]        entry_reset_id      ;
    logic                                entry_reset_vld     ;
    logic [TBU_NUM-1          :0]        entry_reset_arbiter ;
    logic [CUSTOM_DATA_WIDTH-1:0]        reset_data          ;
    logic                                reset_valid         ;
    logic [TBU_NUM_WIDTH-1    :0]        reset_id            ;
    logic [TBU_IDX_W-1:0]          allocate_id         ;
    logic                                allocate_vld        ;
    ...
    //=================================================
    generate
        if (TBU_NUM == 1) begin : g_single_entry_alloc
            assign allocate_oh  = entry_idle;
            assign allocate_id  = '0;
            assign allocate_vld = entry_idle[0];
        end else begin : g_multi_entry_alloc
            fcip_lead_one #(
                .ENTRY_NUM (TBU_NUM)
    ...
    generate
        for (genvar i=0; i<TBU_NUM; i++) begin: rob_entry
            pcie_eth_dti_pr_rob_state_entry #(
                .TBU_NUM             (TBU_NUM             ),
    ...
    generate
        if (TBU_NUM == 1) begin : g_single_entry_reset
            assign entry_reset     = entry_reset_arbiter;
            assign entry_reset_id  = '0;
            assign entry_reset_vld = entry_reset_arbiter[0];
        end else begin : g_multi_entry_reset
            fcip_lead_one #(
                .ENTRY_NUM (TBU_NUM)
    ...
    always_comb begin: trans_num_overflow_ready
        ...
        for (int i=0; i<TBU_NUM; i++) begin
            if (!entry_idle[i] && (entry_tid[i] == req_tid)) begin
                entry_tid_hit = 1'b1;
                entry_ready = ~trans_num_overflow[i];
            end
        end

        if ((m_msg_type == DTI_TBU_CONDIS_REQ) && m_state && !entry_tid_hit) begin
            entry_ready = allocate_vld;
        end
    end
    ...
    // dti to conv
    assign req_valid   = partial_reset ? reset_valid : (req_tvalid && ~stall && entry_ready);
    assign req_data    = partial_reset ? reset_data : req_tdata;
    assign req_keep    = partial_reset ? reset_keep : req_tkeep;
    assign req_tid     = partial_reset ? reset_id : TBU_NUM_WIDTH'(req_ttid + TBU_NUM_WIDTH'(TTID_BASE));
