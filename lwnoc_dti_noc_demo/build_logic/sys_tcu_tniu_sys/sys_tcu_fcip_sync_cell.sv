    ...
    generate
        for(genvar i = 0; i < DATA_WIDTH; i=i+1) begin : gen_sync_cell
            if (RST_VALUE[i]==0) begin
                `ifdef sys_tcu_FCIP_SYNC_CELL_ARST_MODULE_NAME
                    sys_tcu_`FCIP_SYNC_CELL_ARST_MODULE_NAME #(
                        .VT_TYPE(VT_TYPE            ),
                        .SYN_NUM(SYN_STAGE          )
                    ) u_sync_arst (
                        .D      (d[i]               ),
                ...
                `ifdef sys_tcu_FCIP_SYNC_CELL_ASET_MODULE_NAME
                    sys_tcu_`FCIP_SYNC_CELL_ASET_MODULE_NAME #(
                        .VT_TYPE(VT_TYPE            ),
                        .SYN_NUM(SYN_STAGE          )
                    ) u_sync_aset (
                        .D      (d[i]               ),
