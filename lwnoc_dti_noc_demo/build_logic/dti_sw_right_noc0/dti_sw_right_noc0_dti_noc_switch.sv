...
//
// Limitations:
//   - threshold signals are tied to 1'b1 (backpressure not used).
//   - qos / tgtid channels are not forwarded (tied 0/1 per convention).
// =============================================================================

module dti_sw_right_noc0_dti_noc_switch #(
    ...
    output logic [NUM_INIU-1:0]                     iniu_rsp_threshold,
    ...
    output logic [NUM_TNIU-1:0]                     tniu_req_threshold,
    ...
    // Local parameters
    // ─────────────────────────────────────────────────────────────────────
    // Grouped payload: {payload, srcid, last}
    localparam int unsigned GPW        = PAYLOAD_WIDTH + TID_WIDTH + 1;
    ...
    // ─────────────────────────────────────────────────────────────────────
    assign iniu_rsp_threshold = {NUM_INIU{1'b1}};
    assign tniu_req_threshold = {NUM_TNIU{1'b1}};
    ...
    generate
        for (gi = 0; gi < NUM_INIU; gi++) begin : g_req_pack
            assign req_gpld_in[gi] = {iniu_req_payload[gi], iniu_req_srcid[gi],
                                      iniu_req_last[gi]};
        end
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_rsp_pack
            assign rsp_gpld_in[gj] = {tniu_rsp_payload[gj], tniu_rsp_srcid[gj],
                                      tniu_rsp_last[gj]};
        end
    endgenerate
    ...
    generate
        for (gj = 0; gj < NUM_TNIU; gj++) begin : g_req_arb
            ...
            assign tniu_req_srcid[gj]   = req_gpld_out[gj][TID_WIDTH:1];
            assign tniu_req_payload[gj] = req_gpld_out[gj][GPW-1:TID_WIDTH+1];
    ...
    generate
        for (gi = 0; gi < NUM_INIU; gi++) begin : g_rsp_arb
            ...
            assign iniu_rsp_srcid[gi]   = rsp_gpld_out[gi][TID_WIDTH:1];
            assign iniu_rsp_payload[gi] = rsp_gpld_out[gi][GPW-1:TID_WIDTH+1];
