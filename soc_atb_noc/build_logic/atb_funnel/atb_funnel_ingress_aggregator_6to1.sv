// atb_funnel_ingress_aggregator_6to1.sv — 6-to-1 per-channel ATB aggregator.

module atb_funnel_ingress_aggregator_6to1 #(
    ...
    output logic [6-1:0] atvalids,
    output logic [6-1:0] afreadys,
    output logic [6-1:0][ATB_ID_WIDTH-1:0] atids,
    output logic [6-1:0][ATB_DATA_WIDTH-1:0] atdatas,
    output logic [6-1:0][3:0] atbytess,
    input  logic [6-1:0] atreadys,
    input  logic [6-1:0] afvalids,
    input  logic [6-1:0] syncreqs
);
    genvar i;
    generate for(i=0;i<6;i++) begin:g
    ...
    generate for(i=0;i<6;i++) begin:u
