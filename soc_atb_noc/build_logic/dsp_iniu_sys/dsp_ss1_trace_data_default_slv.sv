...
//2.
module dsp_ss1_trace_data_default_slv
import dsp_ss1_lw_atb_noc_pack::*;
(
    input   logic                       clk     ,
    input   logic                       rst_n   ,
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        end else if (m_atvalid && !m_atready) begin
            trace_hold_active <= 1'b1;
            hold_atbytes <= m_atbytes;
            hold_atdata <= m_atdata;
            hold_atid <= m_atid;
            hold_atwakeup <= m_atwakeup;
        end
    end
end
...
assign m_atvalid = trace_hold_active ? 1'b1 : (switch_done ? 1'b0 : s_atvalid);
assign s_atready = switch_done ? 1'b1 : (trace_hold_active ? 1'b0 : m_atready);
