module peri_atb_tniu_sys
import peri_lw_atb_noc_pack::*;
import lwnoc_lp_define_package::*;
import lwnoc_lp_struct_package::*;
#(
    parameter integer unsigned FIFO_DEPTH        = `peri_ATB_TNIU_FIFO_DEPTH,
    parameter integer unsigned AUTO_CLEAR_EN     = `peri_ATB_TNIU_AUTO_CLEAR_EN,
    parameter integer unsigned SYNC_STAGE        = `peri_ATB_TNIU_SYNC_STAGE,
    parameter integer unsigned ERR_INT_CNT_WIDTH = `peri_ATB_TNIU_ERR_INT_CNT_WIDTH,
    ...
    output  logic                       atb_tniu_afifo_sb_err,
    output  logic                       atb_tniu_afifo_db_err,
...
//=== Pchannel typed signals (ports are flat vectors) ===
...
//=== Pchannel type casts (flat vectors ↔ typed internal) ===
...
peri_niu_atb_mst #(
    ...
    .afifo_mst_sb_err       (atb_tniu_afifo_sb_err),
    .afifo_mst_db_err       (atb_tniu_afifo_db_err),
...
peri_flush_sync_stall u_flush_sync_stall (
    .clk         (clk_atb_m        ),
    .rst_n       (rstn_atb_m       ),
