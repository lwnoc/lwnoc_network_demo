module fcip_sync_fifo_reg_mimo #(
    ...
    // WRITE REQ channel
    output logic            [WR_WIDTH-1:0]              v_s_rdy                 ,
    input  logic            [WR_WIDTH-1:0]              v_s_vld                 ,
    input  PLD_TYPE                                     v_s_pld [WR_WIDTH-1:0]  ,

    // READ ACK channel
    output logic            [RD_WIDTH-1:0]              v_m_vld                 ,
    input  logic            [RD_WIDTH-1:0]              v_m_rdy                 ,
    output PLD_TYPE                                     v_m_pld [RD_WIDTH-1:0]
);
...
//===========================================================================
    ...
    generate
        for (genvar i=0; i<WR_WIDTH; i=i+1) begin: GEN_WR_REQ_RDY
            assign v_s_rdy[i] = crdt_residue >= CRDT_CNT_WIDTH'(i+1);
        end
    endgenerate
    //
    generate
        for(genvar i = 0; i < RD_WIDTH; i=i+1) begin: GEN_RD_ACK_VLD
            assign v_m_vld[i] = (crdt_cnt>CRDT_CNT_WIDTH'(i));
        end
    endgenerate
...
//===========================================================================
    ...
    generate
        for (genvar i = 0; i < WR_WIDTH; i=i+1) begin: GEN_WR_NUM
            if (i==0)   assign v_write_num_acc[i] = WRITE_NUM_WIDTH'(v_s_vld[i] && v_s_rdy[i]);
            else        assign v_write_num_acc[i] = WRITE_NUM_WIDTH'((v_s_vld[i] && v_s_rdy[i]) + v_write_num_acc[i-1]);
        end
    endgenerate
    ...
    generate
        for(genvar i = 0; i < RD_WIDTH; i=i+1) begin: GEN_RD_NUM
            if(i==0)    assign v_read_num_acc[i] = READ_NUM_WIDTH'(v_m_vld[i] && v_m_rdy[i]);
            else        assign v_read_num_acc[i] = READ_NUM_WIDTH'((v_m_vld[i] && v_m_rdy[i]) + v_read_num_acc[i-1]);
        end
    endgenerate
...
// Read/Write pointer
//===========================================================================
    assign wren = |(v_s_rdy & v_s_vld);
    assign rden = |(v_m_rdy|v_m_vld);
    ...
    generate
        for (genvar i = 0; i < RD_WIDTH; i=i+1) begin: GEN_RD_DATA
            ...
            assign v_m_pld[i]         = queue_entry[v_rd_add_ptr[i][DEPTH_WIDTH-1:0]];
        end
    endgenerate
        ...
        for (genvar i=0; i<DEPTH; i=i+1) begin: GEN_ENTRY_WR_REMAP
            ...
            // init
                ...
                for (integer j=0; j<WR_WIDTH; j=j+1) begin
                    if(v_s_rdy[j] && v_s_vld[j] && v_wr_add_ptr[j][DEPTH_WIDTH-1:0]==DEPTH_WIDTH'(i)) begin
                        wr_entry_data_map[i] = wr_entry_data_map[i] | v_s_pld[j];
